...
import { Platform, StyleSheet, Text, View, TouchableOpacity, Image, ActivityIndicator,Keyboard } from 'react-native';
import { Container, Form, Content, Label, Input, Item ,Left,Header,Right, Button, Body, Title} from 'native-base';
...
if(this.state.companyURL==null || this.state.companyURL=='')
{
  ...
  else if(this.state.companyURL.toLowerCase()!=null && this.state.companyURL.toLowerCase()!='')
  {
...
//alert(str);
  ...
  render() {
    return (
      ...
      <Title allowFontScaling={false} style={styles.Title}>Company URL</Title>
      </Body>
      <Right>
      </Right>
      </Header>
     ...
     <Form>
        ...
        <Input  value={this.state.companyURL} onChangeText={(companyURL) => this.setState({companyURL: companyURL})}/>
        </Item>

        <TouchableOpacity style={{marginTop:50, marginLeft:50, marginRight:50, marginBottom:20}}  onPress={() => this.registerCompany()} >
        <View style={styles.button}>
        <Text style={styles.text}>Next</Text>
        </View>
        </TouchableOpacity>

        <TouchableOpacity style={{marginLeft:50, marginRight: 50,}}  onPress={() => this.cancelCompanyRegisterTxt()}>
        <View style={styles.button}>
