...
import {Image, View, StatusBar, TouchableOpacity, WebView,Text,ScrollView, Alert,ActivityIndicator, ImageBackground, TextInput, Dimensions,TouchableHighlight, PermissionsAndroid, Platform, Modal} from "react-native";
import styles from "../../../js/themes/styles"
import {Header, Left, Right, Body, Title, Container, Content, Button, CheckBox, ListItem, Radio} from 'native-base'
import MapView,{Polygon, Callout, PROVIDER_GOOGLE, PROVIDER_DEFAULT} from 'react-native-maps';
...
export default class Dashboard extends Component {
  ...
  saveQuestionnaire(baseURLApp, this.state.token,
       this.state.mobileUserID, this.state.lastLat, this.state.lastLong, punchType, this.state.employeeEmail, this.state.companyID,
       this.state.questions,  this.state.selectedAttributeID, this.state.selectedPunchAttValue) //this.state.dakarToken,
        .then((res) => {
  ...
  //  selectedAttributeID: 0,
   // selectedPunchAttValue: '', 
    existGeoFence(baseURLApp, this.state.token,
       this.state.mobileUserID, this.state.lastLat, this.state.lastLong, punchType, this.state.employeeEmail, this.state.selectedAttributeID, 
       this.state.selectedPunchAttValue) //this.state.dakarToken,
        .then((res) => {
...
//alert('realm realmPunchInDate' + realmPunchInDate +'todayDate' + todayDate);  
 if(realmPunchInDate.getFullYear() === todayDate.getFullYear() &&
...
if(isPopUpShown)
{
 ...
 getAttributes(baseURLApp, this.state.token, this.state.companyID, this.props.navigation)
 .then((responseAtt) => {
...
getQuestionnaire(baseURLApp, this.state.token, this.state.companyID, this.props.navigation)
  .then((result) => {
    var res = result.data;
...
if(mobileUserID>0 && token!="")
{
  ...
  getGeoFence(baseURLApp, token, mobileUserID).then((res) => {
    var authStr = "Authorization has been denied for this request."
    if(res.Message!=null && res.Message.includes(authStr))
    {
      realm.write(() => {
...
validateDevice(baseURLApp, this.state.token, this.state.mobileUserID, currentVersion, manufacturer, uniqueId).then((res) => {
...
//alert(currentVersion)
  storeAppVersion(baseURLApp, this.state.token, this.state.mobileUserID, currentVersion, manufacturer).then((res) => {
   //alert('here')
  // var intelligentButton = false;
...
if(realmPunchTypeCurrent.length>0)
{
const yourDateMilliseconds = realmPunchTypeCurrent[0].punchDateTime.getTime()
const actualTimeMilliseconds = new Date().getTime()
if (realmPunchTypeCurrent.length > 0 
  && realmPunchTypeCurrent[0].punchInPressed 
  &&  (actualTimeMilliseconds - yourDateMilliseconds > 86400000)) { //24 Hour = 86400000 milliseconds
  var realmPunchTypeCurrent = realm.objects('punchInRecord');
  realm.write(() => {
   realm.create('punchInRecord', {
     punchInRecordID:1,
...
initStateMap() {
  ...
  Platform.OS === 'android' ? {} : { enableHighAccuracy: true, timeout: 1000, maximumAge: 1000,distanceFilter:5  }
  );
...
if(exist)
newPageIndex[index]=true;
else 
newPageIndex[index]=false;
})
...
//onRegionChangeComplete={region => this.setState({ region })}
 ...
 <Callout>
    ...
    <Header  iosStatusbar="light-content" androidStatusBarColor='rgba(0,184,108,1)' style={styles.headerDashboard}>
      <Left  style={{flex: 1}}>
      <Button  transparent onPress={()=> this.props.navigation.openDrawer()}>
    <Icon name='ios-menu' style={styles.iconMenuTop}/>
      </Button>
      </Left>
   ...
   {/*   <Icon name='ios-radio-button-off' style={styles.iconLoc}/>*/}
   <Image source={panel} style={{width:30, height:30, marginTop:13}}/>
     {this.state.lastLat!= null && this.state.lastLong!=null && <Text style={styles.textLoc}><Text>My Pin Location</Text>
     <Text style={styles.textLatLong}> {"\n"}[{this.state.lastLat.toString().substring(0,10)},{this.state.lastLong.toString().substring(0,10)}]</Text></Text>}
      </View>
    </View>
  </Callout>
...
<View style={{paddingLeft:10, paddingRight:10}}>
    ...
    {this.state.currentHour!='' && <Text style={{fontSize:20, fontWeight:'bold', color:'rgba(0,0,0, 0.7)'}}>{this.state.currentHour}</Text>}
    {this.state.alertText=='' && <Text style={{color:'rgba(0,0,0, 0.7)'}}>Nice to see you.</Text>}
    {this.state.alertText!='' && this.state.isSucessPunch && <Text style={styles.sucessTextClock}>{this.state.alertText}</Text>}
    {this.state.alertText!='' && this.state.isErrorPunch && <Text style={styles.errorTextClock}>{this.state.alertText}</Text>}
    </View>
  ...
  {this.state.intelligentButtons && 
   <View style={styles.punchButtonsView}>
    {!this.state.punchInPressed && <TouchableOpacity  style={styles.punchBtnStartDash} onPress={()=> this.punch(1)}>
        <Text style={styles.punchInBtnText}>CLOCK IN</Text>
        </TouchableOpacity>}

 { this.state.punchInPressed && <TouchableOpacity  style={styles.punchBtnendDash} onPress={()=> this.markPunch(2)}>
        <Text style={styles.punchInBtnText}>CLOCK OUT</Text>
        </TouchableOpacity>}
        </View>
        }
    ...
    {!this.state.intelligentButtons && 
      ...
      <TouchableOpacity  style={styles.punchBtnendDash} onPress={()=> this.markPunch(2)}>
          <Text style={styles.punchInBtnText}>CLOCK OUT</Text>
          </TouchableOpacity>
          </View>
     }
...
{this.state.alertText!='' && <View style={styles.alertboxDash}>
 ...
 flexDirection: 'row',
   ...
   {this.state.isAttributes && <View style={styles.questionaireDash}>
 <View>
   <Modal
        animationType="slide"
        transparent={true}
        ...
        <View style={styles.modalViewQuest}>
            <Text style={styles.errorMsg}>{this.state.errorAttributeText}</Text>
            <Text style={styles.questionaireTopText} style={{ textTransform: 'capitalize'}}>{this.state.attributeName}</Text>
            <Text style={styles.questionaireTopText}  style={{ textTransform: 'capitalize'}}>{this.state.attributeDescription}</Text>
  <ScrollView persistentScrollbar={true}>
          <View>
    {this.state.punchAttributes.map((item, itemIndex) => (
       <ListItem noBorder onPress={()=> this.checkAttribute(item.punchAttributeValueID, item.punchAttributeValue)}> 
        <CheckBox onPress={()=> this.checkAttribute(item.punchAttributeValueID, item.punchAttributeValue)} color="green"  checked={item.isChecked}/><Text>{item.punchAttributeValue}</Text>
     </ListItem>
     ))}
       </View>
  </ScrollView>

<TouchableOpacity  disabled={this.state.isSubmitPressed} onPress={()=> this.saveEmployeeAttribute()} style={styles.questSubmit}>
    <Text style={styles.punchInBtnText}>Submit</Text>
    </TouchableOpacity>
    <TouchableOpacity  onPress={()=> this.cancelEmployeeAttribute()} style={styles.questSubmitCancel}>
...
{this.state.isQuestionnaire && <View style={styles.questionaireDash}>
 <View>
   <Modal
        animationType="slide"
        transparent={true}
        ...
        <View style={{justifyContent: "center", alignItems: "center"}}>
           <Icon name='ios-alert'   style={{fontSize:40, color:'#d99e16', justifyContent: "center", alignItems: "center"}} /><Text>{this.state.alertText}</Text>
        </View>
        }
        {this.state.riskScale!='high' && <View style={{justifyContent: "center", alignItems: "center"}}>
           <Icon name='ios-checkmark-circle-outline' style={{fontSize:40, color:'rgba(0,184,108,1)', justifyContent: "center", alignItems: "center"}} /> 
        <Text>{this.state.alertText}</Text>
        </View>}
        <TouchableOpacity  onPress={()=> this.submitQuestRes()} style={styles.questResultSubmit}>
    <Text style={styles.punchInBtnText}>ok</Text>
    </TouchableOpacity>
    </View>}
         {!this.state.isSubmitQuestRes && <View style={styles.modalViewQuest}>
            {this.renderPagination()}
            <Text style={styles.errorMsg}>{this.state.errorQuestionsText}</Text>
            {/*Hi there, a quick questions for you, Over the last 3 days, have beenhad done any of the following: */}
            <Text style={styles.questionaireTopText}>{this.state.headerText}</Text>
         <ScrollView persistentScrollbar={true}>
         
            {this.state.questions.map((item, index) => (
              <View>
        <Text style={styles.questionText} key={index}>{item.text}</Text>
      {item.questionTypeID==1 && <View style={{ flexDirection: 'row',  justifyContent: 'flex-start',alignItems: 'flex-start' }}> 
    {item.options.map((opt, optIndex) => (<ListItem noBorder  onPress={()=> this.checkQuestion(index, opt, optIndex)}>
            <CheckBox  color="green" onPress={()=> this.checkQuestion(index, opt, optIndex)} checked={opt.isChecked}/><Text> {opt.title}</Text>
       </ListItem>
     ))}
 </View>}
       </View>
  ...
  <Text style={styles.declarationText}>  <Icon name='ios-checkmark-circle' style={{width:10, height:10}} />  {this.state.declarationQuest}</Text>


<TouchableOpacity  disabled={this.state.isSubmitPressed} onPress={()=> this.saveEmployeeQuestionnaire()} style={styles.questSubmit}>
    <Text style={styles.punchInBtnText}>Submit</Text>
    </TouchableOpacity>
    <TouchableOpacity  onPress={()=> this.cancelEmployeeQuestionnnaire()} style={styles.questSubmitCancel}>
    <Text style={styles.punchInBtnText}>Cancel</Text>
    </TouchableOpacity>
