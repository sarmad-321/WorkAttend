...
export default class Settings extends Component {
  ...
  changeUserPassword() {
    ...
    if (
      ...
      this.state.newPass.trim().length < 6 &&
      this.state.newPassConfirm.trim().length < 6
    ) {
      this.setState({
        errorMsg: 'Password should be atleast 6 characters. Try again.',
        isError: true,
        isLoading: false,
      });
    } else if (this.state.newPass.trim() == this.state.oldPass.trim()) {
      this.setState({
        errorMsg: 'New and old password should not match.',
        isError: true,
        isLoading: false,
      });
    } else if (
      /\s/.test(this.state.newPass) ||
      /\s/.test(this.state.newPassConfirm) ||
      /\s/.test(this.state.oldPass)
    ) {
      // It has any kind of whitespace
      this.setState({
        errorMsg: 'Password cannot contain blank spaces.',
        isError: true,
        isLoading: false,
      });
    } else {
      ...
      changePassword(
        ...
        if (!result) {
          this.setState({
            errorMsg: 'Current Password is not correct. Something went wrong.',
            isError: true,
            isLoading: false,
          });
        }
        if (result) {
          getAppDetail(baseURLApp, mobileAPIToken).then(res => {
            //alert(JSON.stringify(res));
            if (res != null) {
              var pathArray = baseURLApp.split('/api/resource');
    ...
    return (
      <Container style={{backgroundColor: '#ffff'}}>
        ...
        <ScrollView style={{margin: 15}}>
          ...
          <Text style={styles.labelSubHeading}>
