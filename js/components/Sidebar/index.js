...
import {Content, Text, List, ListItem, Container,	Left,	Right,	Badge,	Button,	View,	StyleProvider,	getTheme,	variables,	Body,Header,} from "native-base";
	...
	navigateScreen(route)
	{
	  NetInfo.isConnected.fetch().then(isConnected => {
		if(isConnected)
		{
			...
			Alert.alert('Internet Connection Required', 'Kindly connect to the internet via your cellular data plan or WiFi'); 
		}
})
.catch((error) => {
	console.error(error);
...
else 
{
 alert('Something went wrong. Unable to log out.')
	...
	render() {
		return (
<Container>
<Content bounces={false} style={{ flex: 1, top: -1, backgroundColor: 'rgba{255, 255, 255, 0.6}' }}>
...
<View style={{flexDirection: 'row', paddingBottom:10,  paddingLeft:20, }}>

<View style={{flexDirection: 'column', paddingBottom:10, justifyContent:'center' }}>
<Image source={map} style={{height: 40, width:38}} />
</View>

<View style={{flexDirection: 'column', paddingBottom:10, justifyContent:'center' }}>
<View  style={{flexDirection: 'row', }}>
<Text style={styles.headerTextColorOne}>WORK</Text><Text style={styles.headerTextSecondColor}>ATTEND</Text>
</View>
</View>
</View>
</View>
...
<Text  allowFontScaling={false} style={{padding:3}}></Text>
				<ListItem style={styles.listItem} icon  onPress={() => this.props.navigation.navigate('Dashboard')}>
				<Left>
				<MaterialCommunityIcons name='drag' style={styles.menuIcons}/>
				</Left>
				<Body style={styles.listItemNoBorder}>
				<Text allowFontScaling={false} style={styles.textDrawer}>{this.state.DashboardLabel}</Text>
				</Body>
				...
				<ListItem style={styles.listItem} icon onPress={() => this.props.navigation.navigate('History')}>
				<Left>
				<EvilIcons name='clock' style={styles.menuIcons}/>
				</Left>
				<Body style={styles.listItemNoBorder}>
				<Text allowFontScaling={false} style={styles.textDrawer}>{this.state.HistoryLabel}</Text>
				</Body>
				<Right style={styles.listItemNoBorder}></Right>
				</ListItem> 
				...
				<ListItem style={styles.listItem} icon onPress={() => this.props.navigation.navigate('Profile')}>
				<Left>
				<EvilIcons name='user' style={styles.menuIcons}/>
				</Left>
				<Body style={styles.listItemNoBorder}>
				<Text allowFontScaling={false} style={styles.textDrawer}>{this.state.ProfileLabel}</Text>
				</Body>
				<Right style={styles.listItemNoBorder}></Right>
				</ListItem>

	         	<ListItem style={styles.listItem} icon  onPress={() => this.props.navigation.navigate('Settings')}>
				<Left>
				<EvilIcons name='gear' style={styles.menuIcons}/>
				</Left>
				<Body style={styles.listItemNoBorder}>
				<Text allowFontScaling={false} style={styles.textDrawer}>{this.state.changePassLabel}</Text>
				</Body>
				<Right style={styles.listItemNoBorder}></Right>
				</ListItem>

				<ListItem style={styles.listItem} icon  onPress={() => this.logOut()}>
				<Left>
				<EvilIcons name='arrow-left' style={styles.menuIcons}/>
				</Left>
				<Body style={styles.listItemNoBorder}>
				<Text allowFontScaling={false} style={styles.textDrawer}>{this.state.LogoutLabel}</Text>
				</Body>
				<Right style={styles.listItemNoBorder}></Right>
				</ListItem>
				</List>
				...
				<View style={{flexDirection: 'row', position:'absolute', bottom:0,  alignSelf:'center', margin:20}}>
